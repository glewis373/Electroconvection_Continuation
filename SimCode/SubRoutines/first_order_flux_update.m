function flux_new = first_order_flux_update(flux_now,flux_nonl_now,riprime,in_speed,Pr,It,D,D2,dt,Nc)
...
% The time-stepping is explicit on the nonlinear terms and implicit on the
% linear terms:
%
% (flux_new-flux_now)/dt 
%             = Pr Delta_r flux_new - Pr/r^2 flux_new + flux_nonl_now 
%
...
%
% So we have 
%   flux_new - flux_now = dt*Pr*Delta_r flux_new - dt*Pr/r^2 flux_new +
%                                    dt*flux_nonl_now
% flux_new - dt*Pr*Delta_r flux_new + dt*Pr/r^2 flux_new 
%                       = flux_now + dt*flux_nonl_now
% A flux_new = B
%
% where A := I - dt*Pr*(Lop - 1/r^2*I)    
% and B = flux_now + dt*flux_nonl_now.
...
B = flux_now(2:Nc,1) + dt*flux_nonl_now(2:Nc,1);
