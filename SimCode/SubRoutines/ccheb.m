% function [T_inv,c_int,D,DD,x] = cheb(N)
% given N, this produces 
% T_inv = the (N+1)x(N+1) matrix such that f_cheb = T_inv*f defines the Chebyshev
%            coefficients from the sample values f and
% c_int = the vector of weights used to do integration w/ spectral accuracy
% D = the (N+1)x(N+1) differentiation matrix.
% DD = the (N+1)x(N+1) matrix for the second derivative.
% x = the N+1 collocation points in [-1,1]
%
function [T_inv,c_int,D,DD,x] = ccheb(N)
...
for i = 1:N+1
    for j = 1:N+1 
        T_inv(i,j) = 2*cos((i-1)*(j-1)*pi/N)/(c(i)*c(j)*N);
    end
end
...
% For integration w/ spectral accuracy
ii = 1:N+1;
c_int = -mod(ii,2).*2./((ii-1).^2-1);
c_int(2) = 0;
% int_f = dot(c_int,T_inv*f);
